...
const MARKETPLACE_COLLECTION = "advBankMarketplace";
...
const MARKETPLACE_FEE_PERCENT = 0.02;
const CREDIT_LINE_DAILY_INTEREST = 0.001;
...
module.exports = {
    config: { name: "bank", aliases: ["ab", "anchestor"], version: "15.0", author: "Mahi-- & Gemini", role: 0, countDown: 3, shortDescription: { en: "The complete Anchestor Bank financial system." }, longDescription: { en: "The ultimate banking experience with a huge array of financial tools and games." }, category: "economy" },
    ...
    onStart: async function ({ args, message, event, api, usersData }) {
        ...
        if (userBankInfo.loan.amount > 0 && userBankInfo.loan.dueDate && new Date() > new Date(userBankInfo.loan.dueDate)) { const now = new Date(); const twoHours = 2 * 60 * 60 * 1000; if (!userBankInfo.lastLoanWarning || (new Date() - new Date(userBankInfo.lastLoanWarning).getTime()) > twoHours) { message.reply(`🚨 ${toBoldUnicode("Loan Payment Overdue!")}\nYour loan of ${formatNumber(userBankInfo.loan.amount)} is past its due date. Late payments may negatively affect your credit score. Use '${p}bank payloan' to make a payment.`); userBankInfo.lastLoanWarning = now; await updateUserBankData(senderID, userBankInfo, db); } }
        ...
        switch (command) {
            ...
            case "auction": {
                ...
                if (subCmd === 'status') {
                    ...
                    let statusMsg = `${toBoldUnicode("🔥 Live Auction! 🔥")}\n\n` +
                                    `${toBoldUnicode("Item:")} ${currentAuction.itemName}\n` +
                                    `${toBoldUnicode("Current Bid:")} ${formatNumber(currentAuction.currentBid)}\n` +
                                    `${toBoldUnicode("Highest Bidder:")} ${currentAuction.highestBidderName}\n` +
                                    `${toBoldUnicode("Time Left:")} ${minutes}m ${seconds}s\n\n` +
                ...
                if (subCmd === 'bid') {
                    ...
                    return message.reply(`You are now the highest bidder for the ${toBoldUnicode(currentAuction.itemName)} with a bid of ${formatNumber(bidAmount)}!`);
                }
            ...
            case "business": {
                const subCmd = args[1]?.toLowerCase();
                if (subCmd === 'list') { let listMsg = toBoldUnicode("🏢 Businesses For Sale 🏢\n\n"); availableBusinesses.forEach(b => listMsg += `${toBoldUnicode(b.id)}: ${b.name}\nCost: ${formatNumber(b.cost)}\nIncome: ~${formatNumber(b.baseIncome)}/hr\n---\n`); return message.reply(listMsg + `Use ${p}bank business buy <ID>`); }
                if (subCmd === 'buy') { const businessId = args[2]?.toUpperCase(); const businessToBuy = availableBusinesses.find(b => b.id === businessId); if (!businessToBuy) return message.reply(`Invalid business ID.`); if (userBankInfo.businesses.some(b => b.businessId === businessId)) return message.reply("You already own this type of business."); if (businessToBuy.cost > userCash) return message.reply(`Insufficient cash. You need ${formatNumber(businessToBuy.cost)}.`); await usersData.set(senderID, { money: userCash - businessToBuy.cost }); userBankInfo.businesses.push({ businessId: businessToBuy.id, level: 1, lastCollected: new Date() }); await awardAchievement('FIRST_BIZ'); await updateUserBankData(senderID, userBankInfo, db); await logAudit(db, "BUSINESS_BUY", event, {business: businessId, cost: businessToBuy.cost}); return message.reply(`Congratulations! You are now the owner of ${toBoldUnicode(businessToBuy.name)}.`); }
                if (subCmd === 'portfolio') { if (userBankInfo.businesses.length === 0) return message.reply("You do not own any businesses."); let portMsg = toBoldUnicode("📈 Your Business Portfolio 📈\n\n"); userBankInfo.businesses.forEach(owned => { const details = availableBusinesses.find(b => b.id === owned.businessId); const currentLevel = owned.level || 1; const incomeMultiplier = BUSINESS_UPGRADES[currentLevel][1]; const currentIncome = details.baseIncome * incomeMultiplier; portMsg += `${toBoldUnicode(details.name)} (Lvl ${currentLevel})\nIncome: ~${formatNumber(currentIncome)}/hr\n---\n`; }); return message.reply(portMsg + `Use ${p}bank business collect to get your profits.`); }
                if (subCmd === 'collect') { if (userBankInfo.businesses.length === 0) return message.reply("You do not own any businesses to collect profits from."); let totalProfit = 0; const now = new Date(); userBankInfo.businesses.forEach(owned => { const details = availableBusinesses.find(b => b.id === owned.businessId); const currentLevel = owned.level || 1; const incomeMultiplier = BUSINESS_UPGRADES[currentLevel][1]; const currentIncome = details.baseIncome * incomeMultiplier; const hoursSinceCollected = (now.getTime() - new Date(owned.lastCollected).getTime()) / 36e5; const profit = Math.floor(hoursSinceCollected * currentIncome); totalProfit += profit; owned.lastCollected = now; }); if (totalProfit <= 0) return message.reply("It's too soon to collect profits. Wait a while longer."); let finalProfit = totalProfit; if (userBankInfo.corporation.id) { const corpData = await db.collection(CORP_COLLECTION).findOne({ _id: new ObjectId(userBankInfo.corporation.id) }); if (corpData && corpData.sabotageDebuff && new Date() < new Date(corpData.sabotageDebuff.endTime)) { finalProfit *= (1 - corpData.sabotageDebuff.effect); } } const taxAmount = finalProfit * INCOME_TAX_RATE; const netProfit = finalProfit - taxAmount; userBankInfo.tax.reportableIncome = (userBankInfo.tax.reportableIncome || 0) + finalProfit; userBankInfo.bank += netProfit; userBankInfo.taxPaid += taxAmount; await updateUserBankData(senderID, userBankInfo, db); await logAudit(db, "BUSINESS_COLLECT", event, {amount: netProfit, tax: taxAmount}); return message.reply(`You collected a total of ${formatNumber(finalProfit)} from your businesses. After a ${formatNumber(taxAmount)} tax, ${formatNumber(netProfit)} has been deposited into your bank.`); }
                if (subCmd === 'upgrade') { const bizId = args[2]?.toUpperCase(); const ownedBiz = userBankInfo.businesses.find(b => b.businessId === bizId); if (!ownedBiz) return message.reply("You do not own this business."); const currentLevel = ownedBiz.level || 1; const nextLevel = currentLevel + 1; if (!BUSINESS_UPGRADES[nextLevel]) return message.reply("This business is already at its maximum level."); const upgradeCost = BUSINESS_UPGRADES[nextLevel][0]; if (userBankInfo.bank < upgradeCost) return message.reply(`You need ${formatNumber(upgradeCost)} in your bank to upgrade.`); userBankInfo.bank -= upgradeCost; ownedBiz.level = nextLevel; await updateUserBankData(senderID, userBankInfo, db); return message.reply(`Successfully upgraded your business to Level ${nextLevel}!`); }
                if (subCmd === 'manage') { const bizId = args[2]?.toUpperCase(); const ownedBiz = userBankInfo.businesses.find(b => b.businessId === bizId); if (!ownedBiz) return message.reply("You do not own this business."); const details = availableBusinesses.find(b => b.id === ownedBiz.businessId); const currentLevel = ownedBiz.level || 1; const incomeMultiplier = BUSINESS_UPGRADES[currentLevel][1]; const currentIncome = details.baseIncome * incomeMultiplier; const nextLevel = currentLevel + 1; let upgradeInfo = "Max Level Reached"; if (BUSINESS_UPGRADES[nextLevel]) { upgradeInfo = `Upgrade to Lvl ${nextLevel} for ${formatNumber(BUSINESS_UPGRADES[nextLevel][0])}`; } let manageMsg = `${toBoldUnicode("Business Management")}\n\n` + `${toBoldUnicode("Name:")} ${details.name}\n` + `${toBoldUnicode("Level:")} ${currentLevel}\n` + `${toBoldUnicode("Current Income:")} ${formatNumber(currentIncome)}/hr\n` + `${toBoldUnicode("Next Upgrade:")} ${upgradeInfo}`; return message.reply(manageMsg); }
            ...
            case "corp": {
                ...
                if(subCmd === 'create'){ const corpName = args.slice(2).join(" "); if(!corpName) return message.reply("Please provide a name for your corporation."); if(userBankInfo.corporation.id) return message.reply("You are already in a corporation."); if(userBankInfo.bank < CORP_CREATION_FEE) return message.reply(`You need ${formatNumber(CORP_CREATION_FEE)} to found a corporation.`); userBankInfo.bank -= CORP_CREATION_FEE; const newCorp = { name: corpName, ceoId: senderID, members: [senderID], treasury: 0, level: 1, defenseEndTime: null, sabotageDebuff: null, intel: {}, createdAt: new Date(), heist: { active: false } }; const result = await corpCollection.insertOne(newCorp); userBankInfo.corporation = { id: result.insertedId.toString(), name: corpName, rank: 'CEO' }; await updateUserBankData(senderID, userBankInfo, db); await awardAchievement('CORP_FOUNDER'); return message.reply(`You have successfully founded the corporation ${toBoldUnicode(corpName)}!`); }
                if(!userBankInfo.corporation.id && subCmd !== 'join') return message.reply("You are not part of a corporation.");
                if(subCmd === 'join'){ const corpName = args.slice(2).join(" "); const corpToJoin = await corpCollection.findOne({ name: corpName, invites: senderID }); if(!corpToJoin) return message.reply("You have not been invited to this corporation or it does not exist."); userBankInfo.corporation = { id: corpToJoin._id.toString(), name: corpToJoin.name, rank: 'Member' }; await corpCollection.updateOne({ _id: corpToJoin._id }, { $pull: { invites: senderID }, $push: { members: senderID } }); await updateUserBankData(senderID, userBankInfo, db); return message.reply(`You have successfully joined ${toBoldUnicode(corpToJoin.name)}!`); }
                ...
                if(subCmd === 'members'){ let memberList = `${toBoldUnicode(corpData.name + " - Member List")}\n\n`; for(const memberId of corpData.members){ const memberName = await usersData.getName(memberId); const memberData = await getUserBankData(memberId, db); memberList += `- ${memberName} [${memberData.corporation.rank}]\n`; } return message.reply(memberList); }
                if(subCmd === 'heist'){ if(userBankInfo.corporation.rank !== 'CEO') return message.reply("Only the CEO can initiate a heist."); if(corpData.heist && corpData.heist.active) return message.reply("A heist is already in progress!"); const setupCost = 50000000; if(corpData.treasury < setupCost) return message.reply(`The corporation needs ${formatNumber(setupCost)} in its treasury to start a heist.`); const newHeist = { active: true, stage: 1, successPoints: 0, startTime: new Date() }; await corpCollection.updateOne({ _id: corpData._id }, { $set: { heist: newHeist }, $inc: { treasury: -setupCost }}); return message.reply(`${toBoldUnicode("Bank Heist Initiated!")}\n\nYour corporation has paid ${formatNumber(setupCost)} to begin the heist on the Anchestor Reserve. You have 1 hour to complete all stages.\n\n${toBoldUnicode("Stage 1: RECON")}\nAll members must now use \`${p}bank corp heist recon\` to gather intel. Good luck.`); }
                if (subCmd === 'defend') { if (userBankInfo.corporation.rank !== 'CEO') return message.reply("Only the CEO can authorize defense spending."); const cost = 10000000; if (corpData.treasury < cost) return message.reply(`The treasury needs ${formatNumber(cost)} to bolster defenses.`); const defenseDuration = 12 * 3600000; await corpCollection.updateOne({ _id: corpData._id }, { $set: { defenseEndTime: new Date(Date.now() + defenseDuration) }, $inc: { treasury: -cost } }); return message.reply(`Corporate defenses have been bolstered for 12 hours.`); }
                if (subCmd === 'intel') { const targetCorpName = args.slice(2).join(" "); const cost = 2500000; if (corpData.treasury < cost) return message.reply(`The treasury needs ${formatNumber(cost)} for this intel operation.`); const targetCorp = await corpCollection.findOne({ name: targetCorpName }); if (!targetCorp) return message.reply("Target corporation not found."); await corpCollection.updateOne({ _id: corpData._id }, { $set: { [`intel.${targetCorp._id.toString()}`]: { bonus: 0.15, expiry: new Date(Date.now() + 3 * 3600000) } }, $inc: { treasury: -cost } }); return message.reply(`Intel gathered on ${toBoldUnicode(targetCorpName)}. You now have an increased chance of sabotage for the next 3 hours.`); }
                if (subCmd === 'sabotage') { const targetCorpName = args.slice(2).join(" "); const cost = 25000000; if (corpData.treasury < cost) return message.reply(`The treasury needs ${formatNumber(cost)} for this sabotage operation.`); const targetCorp = await corpCollection.findOne({ name: targetCorpName }); if (!targetCorp) return message.reply("Target corporation not found."); let successChance = 0.40; if (targetCorp.defenseEndTime && new Date() < new Date(targetCorp.defenseEndTime)) { successChance -= 0.25; } if (corpData.intel && corpData.intel[targetCorp._id.toString()] && new Date() < new Date(corpData.intel[targetCorp._id.toString()].expiry)) { successChance += corpData.intel[targetCorp._id.toString()].bonus; } await corpCollection.updateOne({ _id: corpData._id }, { $inc: { treasury: -cost } }); if (Math.random() < successChance) { const sabotageDuration = 6 * 3600000; const sabotageEffect = { effect: 0.5, endTime: new Date(Date.now() + sabotageDuration) }; await corpCollection.updateOne({ _id: targetCorp._id }, { $set: { sabotageDebuff: sabotageEffect } }); return message.reply(`✅ Success! Sabotage mission against ${toBoldUnicode(targetCorpName)} was successful. Their business income will be reduced by 50% for 6 hours.`); } else { return message.reply(`❌ Failure! Your sabotage attempt was detected and failed.`); } }
            ...
            case "creditline": {
                const subCmd = args[1]?.toLowerCase();
                const cl = userBankInfo.creditLine;
                if (subCmd === 'apply') { if (cl.issued) return message.reply("You already have an active credit line."); if (userBankInfo.creditScore < 720) return message.reply("Your credit score is too low to apply. A score of 720 or higher is required."); const stockValue = (userBankInfo.stocks || []).reduce((s, st) => s + (stockMarket[st.symbol]?.price * st.shares || 0), 0); const netWorth = userBankInfo.bank + userCash + stockValue; const limit = Math.floor(netWorth * 0.1) + (userBankInfo.creditScore * 100); cl.issued = true; cl.limit = limit; cl.lastInterestAccrual = new Date(); await updateUserBankData(senderID, userBankInfo, db); return message.reply(`Congratulations! Your application for a Credit Line was approved with a limit of ${formatNumber(limit)}.`); }
                if (!cl.issued) return message.reply("You do not have a credit line. Use 'creditline apply'.");
                const lastAccrual = new Date(cl.lastInterestAccrual || Date.now()); const daysPassed = (Date.now() - lastAccrual.getTime()) / 86400000; if (daysPassed >= 1 && cl.outstandingBalance > 0) { const interest = cl.outstandingBalance * CREDIT_LINE_DAILY_INTEREST * Math.floor(daysPassed); cl.outstandingBalance += interest; cl.lastInterestAccrual = new Date(); await updateUserBankData(senderID, userBankInfo, db); }
                if (subCmd === 'status') { return message.reply(`${toBoldUnicode("Credit Line Status")}\n\n${toBoldUnicode("Limit:")} ${formatNumber(cl.limit)}\n${toBoldUnicode("Available:")} ${formatNumber(cl.limit - cl.outstandingBalance)}\n${toBoldUnicode("Balance Owed:")} ${formatNumber(cl.outstandingBalance)}`); }
                if (subCmd === 'draw') { const amount = parseFloat(args[2]); if (isNaN(amount) || amount <= 0) return message.reply("Invalid amount."); if (amount > (cl.limit - cl.outstandingBalance)) return message.reply("This amount exceeds your available credit."); cl.outstandingBalance += amount; userBankInfo.bank += amount; await updateUserBankData(senderID, userBankInfo, db); return message.reply(`Successfully drew ${formatNumber(amount)} from your credit line.`); }
                if (subCmd === 'repay') { const amount = parseFloat(args[2]); if (isNaN(amount) || amount <= 0) return message.reply("Invalid amount."); if (amount > userBankInfo.bank) return message.reply("Insufficient funds in your bank account."); const payment = Math.min(amount, cl.outstandingBalance); cl.outstandingBalance -= payment; userBankInfo.bank -= payment; await updateUserBankData(senderID, userBankInfo, db); return message.reply(`Successfully repaid ${formatNumber(payment)} to your credit line. New balance: ${formatNumber(cl.outstandingBalance)}.`); }
                return message.reply(`Invalid creditline command. Use 'status', 'apply', 'draw', or 'repay'.`);
            }
            case "crypto": {
                const subCmd = args[1]?.toLowerCase();
                ...
                if (subCmd === 'setup') {
                    ...
                    if (args[2]?.toLowerCase() === 'buy' && buyLevel) {
                        ...
                        await updateUserBankData(senderID, userBankInfo, db);
                        return message.reply(`Congratulations! You've purchased the ${toBoldUnicode(rigToBuy.name)}. Use '${p}bank crypto mine' to start mining!`);
                    }
            ...
            case "market": {
                const marketplace = db.collection(MARKETPLACE_COLLECTION);
                const subCmd = args[1]?.toLowerCase();
                if (subCmd === 'list') {
                    const listings = await marketplace.find().limit(20).toArray();
                    if (listings.length === 0) return message.reply("The player marketplace is currently empty.");
                    let listMsg = `${toBoldUnicode("Player Marketplace")}\n\n`;
                    for (const l of listings) {
                        const sellerName = await usersData.getName(l.sellerId) || "Unknown";
                        const assetName = l.assetType === 'property' ? propertyAssets.find(p => p.id === l.assetId)?.name : availableBusinesses.find(b => b.id === l.assetId)?.name;
                        listMsg += `ID: ${l._id.toString().slice(-6)} | ${toBoldUnicode(assetName)}\n` + `Type: ${l.assetType}\n` + `Price: ${formatNumber(l.price)} | Seller: ${sellerName}\n---\n`;
                    }
                    return message.reply(listMsg + `Use '${p}bank market buy <ID>' to purchase.`);
                }
                if (subCmd === 'sell') {
                    const assetType = args[2]?.toLowerCase();
                    const assetId = args[3]?.toUpperCase();
                    const price = parseFloat(args[4]);
                    if (!['property', 'business'].includes(assetType) || !assetId || isNaN(price) || price <= 0) return message.reply("Invalid format. Use: market sell <property/business> <ID> <price>");
                    let assetIndex = -1;
                    if (assetType === 'property') assetIndex = userBankInfo.properties.findIndex(p => p.assetId === assetId);
                    else assetIndex = userBankInfo.businesses.findIndex(b => b.businessId === assetId);
                    if (assetIndex === -1) return message.reply("You do not own this asset.");
                    const [assetToSell] = assetType === 'property' ? userBankInfo.properties.splice(assetIndex, 1) : userBankInfo.businesses.splice(assetIndex, 1);
                    await marketplace.insertOne({ sellerId: senderID, assetType, assetId: assetToSell.assetId || assetToSell.businessId, assetData: assetToSell, price, listedDate: new Date() });
                    await updateUserBankData(senderID, userBankInfo, db);
                    return message.reply("Your asset has been listed on the marketplace.");
                }
                if (subCmd === 'buy') {
                    const listingId = args[2];
                    if (!listingId) return message.reply("Please provide a listing ID.");
                    const listing = await marketplace.findOne({ _id: new ObjectId(listingId) });
                    if (!listing) return message.reply("Listing not found.");
                    if (listing.sellerId === senderID) return message.reply("You cannot buy your own listing.");
                    if (listing.price > userBankInfo.bank) return message.reply("You do not have enough money in your bank for this purchase.");
                    userBankInfo.bank -= listing.price;
                    const fee = listing.price * MARKETPLACE_FEE_PERCENT;
                    const sellerProceeds = listing.price - fee;
                    let sellerData = await getUserBankData(listing.sellerId, db);
                    if (sellerData) { sellerData.bank += sellerProceeds; await updateUserBankData(listing.sellerId, sellerData, db); }
                    if (listing.assetType === 'property') userBankInfo.properties.push(listing.assetData);
                    else userBankInfo.businesses.push(listing.assetData);
                    await marketplace.deleteOne({ _id: listing._id });
                    await updateUserBankData(senderID, userBankInfo, db);
                    return message.reply(`Successfully purchased asset for ${formatNumber(listing.price)}.`);
                }
                return message.reply(`Invalid market command. Use 'list', 'sell', or 'buy'.`);
            }
            ...
            case "research": {
                ...
                return message.reply(`${toBoldUnicode("Anchestor AI Research Report")}\n${toBoldUnicode("Asset:")} ${assetName} (${symbol})\n\n${aiResponse}\n\n${toBoldUnicode("A fee of " + formatNumber(RESEARCH_FEE) + " has been deducted from your account. This is not financial advice.")}`);
            }
